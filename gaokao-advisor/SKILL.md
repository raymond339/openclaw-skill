...
**重要：** 若考生省份为浙江，应优先使用本地数据文件：
- `~/.openclaw/workspace/skills/gaokao-advisor/data/zhejiang_2025_一段.csv`（17890条记录）
- `~/.openclaw/workspace/skills/gaokao-advisor/data/zhejiang_2025_二段.csv`（7096条记录）

数据字段：`学校代号, 学校名称, 专业代号, 专业名称, 计划数, 分数线, 位次`

**匹配逻辑：**
1. 根据考生科类（文/理）判断使用一段还是二段数据（理科生优先看一段）
2. 根据分数/位次筛选目标区间（冲刺：分数线≥考生分-5；稳妥：考生分-20～考生分-5；保底：考生分-20以下）
3. 按专业方向过滤，再按区域过滤
4. 按分数线从高到低排序，每个梯度取至少5所

**若本地数据无法满足需求（如省外高校、跨省考生），再调用联网搜索补充。**

### 3. 联网检索（补充）
调用 AI 的联网搜索能力，获取以下实时或最新数据（浙江考生优先用本地数据）：
